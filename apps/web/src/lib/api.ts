  ...
  hooks: {
    beforeRequest: [
      (request) => {
        ...
        const staffToken = useStaffStore.getState().token;
      ...
      async (request, options, response) => {
        if (response.status === 401) {
          ...
          try {
           ...
           if (body.message) {
               errMessage = typeof body.message === 'string' ? body.message : JSON.stringify(body.message);
           } else if (body.error) {
               errMessage = typeof body.error === 'string' ? body.error : JSON.stringify(body.error);
           }
