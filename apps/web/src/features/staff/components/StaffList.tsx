...
import { Plus, User, BadgeCheck, Pencil, Trash2, Banknote, ShieldCheck } from 'lucide-react';
                ...
                >
                    ...
                    <div className="relative h-32 w-full overflow-hidden bg-slate-100">
                        {member.image ? (
                            ...
                            <div className="flex h-full w-full items-center justify-center bg-slate-50 relative overflow-hidden">
                                <div className="z-10 h-16 w-16 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center text-primary font-bold text-2xl border-2 border-white shadow-sm">
                    ...
                    <div className="p-4 space-y-4">
                        ...
                        <div className="flex items-center justify-between text-xs text-muted-foreground">
                            <span className="font-mono bg-slate-100 px-2 py-1 rounded">ID: {member.contact}</span>
                            <span className="text-xs">Joined {new Date(member.createdAt).toLocaleDateString()}</span>
                        </div>

                        {member.role === 'owner' && member.salaryEnabled === false ? (
                             <div className="pt-3 border-t border-dashed border-slate-200">
                                 <div className="flex flex-col items-center justify-center p-4 bg-gradient-to-br from-primary/5 to-primary/10 rounded-xl border border-primary/20 relative overflow-hidden">
                                     <ShieldCheck className="absolute -right-4 -bottom-4 w-24 h-24 text-primary/5 rotate-12" />
                                     <div className="bg-white/80 backdrop-blur-sm p-2 rounded-full mb-2 shadow-sm relative z-10">
                                         <BadgeCheck className="w-6 h-6 text-primary" />
                                     </div>
                                     <span className="text-sm font-bold text-slate-700 relative z-10">Store Owner Access</span>
                                     <span className="text-xs text-muted-foreground mt-1 text-center relative z-10">Full administrative privileges enabled. Salary tracking is hidden.</span>
                                 </div>
                             </div>
                        ) : (isOwner || member.role === 'manager') && (member.salaryEnabled !== false) ? (
                            <div className="pt-3 border-t border-dashed border-slate-200">
