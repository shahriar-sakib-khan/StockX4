...
import { Search, User, Store, FileText, PackageMinus } from 'lucide-react';
...
export const POSCustomerSection = ({ storeId }: POSCustomerSectionProps) => {
    ...
    const totalDueCylinders = selectedCustomer?.dueCylinders?.reduce((sum: number, item: any) => sum + item.quantity, 0) || 0;
                            ...
                            {selectedCustomer ? (
                                <>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        {totalDueCylinders > 0 && (
                                            <span className="flex items-center gap-1 text-sm font-bold text-orange-600 bg-orange-100 px-2 py-1 rounded-md">
                                                <PackageMinus className="w-4 h-4" /> {totalDueCylinders}
                                            </span>
                                        )}
                                    </div>
                                </>
