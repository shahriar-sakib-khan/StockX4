...
export const GlobalCatalogTab = ({ storeId, selectedBrandIds, onSelectionChange }: GlobalCatalogTabProps) => {
    // Only show GlobalBrands here — custom brands are managed in CustomBrandForm tab
    ...
    // Track which global brands are already linked to this store (isActive)
    const linkedGlobalIds = useMemo(() => {
        return new Set(
            (storeData?.brands || [])
    ...
    const filtered = globalBrands.filter((b: any) =>
    ...
    return (
        <div className="flex flex-col flex-1 min-h-0 gap-4">
            <div className="flex items-center gap-2">
                ...
                <Button variant="outline" onClick={handleBulkAction} disabled={filtered.length === 0} className="min-w-[100px]">
            ...
            <div className="flex-1 min-h-0 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 overflow-y-auto pr-2 pb-2">
                {filtered.map((brand: any) => {
                    ...
                    const alreadyInStore = linkedGlobalIds.has(brand._id);
                        ...
                        >
                            <div className="flex-1 flex flex-col min-w-0">
                                <div className={`flex-1 p-2 flex items-center justify-center relative ${selected ? "bg-emerald-50/50" : "bg-white/50"}`}>
                                    ...
                                    {alreadyInStore && !selected && (
                                        <span className="absolute top-1 left-1 text-[9px] bg-blue-100 text-blue-700 px-1.5 py-0.5 rounded-full font-bold border border-blue-200">In Store</span>
                                    )}
                                </div>
                            ...
                            <div className={`w-12 border-l flex flex-col ${selected ? "border-emerald-200" : ""}`}>
                                <div className={`w-full h-full flex items-center justify-center transition-colors ${
                                    selected ? "bg-emerald-100 text-emerald-700" : "bg-muted/5 text-muted-foreground group-hover:text-primary group-hover:bg-primary/5"
                                }`}>
                ...
                {filtered.length === 0 && (
                    <div className="col-span-full text-center p-8 text-muted-foreground">No brands found matching "{search}"</div>
                )}
            </div>
        </div>
    );
