...
export const CustomBrandForm = ({ onSuccess, initialData, storeId }: { onSuccess: () => void, initialData?: any, storeId: string }) => {
    ...
    const [name, setName] = useState(initialData?.customName || initialData?.name || "");
    const [color, setColor] = useState(initialData?.customColor || initialData?.color || "#EAB308");
    const [logo, setLogo] = useState(initialData?.customLogo || initialData?.logo || "");
    const [cylinderImage, setCylinderImage] = useState(initialData?.customCylinderImage || initialData?.cylinderImage || "");
        ...
        try {
            if (initialData) {
                await update.mutateAsync({
                    storeId,
                    id: initialData._id,
                    data: { customName: name, customColor: color, customLogo: logo, customCylinderImage: cylinderImage }
                });
                ...
                await create.mutateAsync({
                    ...
                    data: {
                        ...
                        customColor: color,
                        customLogo: logo,
                        customCylinderImage: cylinderImage,
                    }
                });
