...
export interface SavedReg extends RegForm { _id: string; productId?: string; }
...
export const RegulatorCard = ({
  ...
  onSave: (id: string, fields: Partial<RegForm>) => void;
  ...
  const setField = (k: keyof RegForm, v: any) => setLocal(p => ({ ...p, [k]: v }));
  const img = item.image || imgMap[item.size] || '';
  const is22 = item.size === '22mm';

  return (
    <div className={`border-2 ${is22 ? 'border-orange-200 hover:border-orange-400' : 'border-yellow-200 hover:border-yellow-400'} bg-white rounded-2xl shadow-md flex flex-col overflow-hidden transition-all`}>
      <div className="relative bg-slate-50 flex items-center justify-center p-3 border-b border-slate-100" style={{ minHeight: 160 }}>
        <span className={`absolute top-2 right-2 text-[10px] font-black px-2 py-0.5 rounded-full ${is22 ? 'bg-orange-100 text-orange-700' : 'bg-yellow-100 text-yellow-700'}`}>
          {item.size}
        </span>
