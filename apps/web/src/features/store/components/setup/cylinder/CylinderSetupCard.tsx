...
const CountBox = ({ label, value, bg, border, labelColor, valueColor, onChange }: {
  ...
  <div className={`${bg} rounded-lg py-2 px-1 flex flex-col items-center justify-center text-center ${border} border min-w-0 shadow-sm flex-1 h-full`}>
    <span className={`text-[9px] font-black ${labelColor} uppercase tracking-wider truncate w-full text-center mb-0.5`}>{label}</span>
    <input
      type="number" min={0} value={value === 0 ? '' : value} placeholder="0"
      onChange={e => onChange(Math.max(0, Number(e.target.value) || 0))}
      className={`w-full text-center text-4xl font-black ${valueColor} leading-none bg-transparent border-none outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`}
    />
  </div>
);
...
const PriceRow = ({ label, value, valueColor, onChange }: {
  ...
  <div className="flex items-center justify-between w-full px-1 gap-1">
    <span className="text-[11px] font-black text-slate-400 uppercase tracking-wide shrink-0">{label}</span>
    <div className="flex items-center gap-0.5 min-w-0">
      <span className={`text-base font-black ${valueColor} shrink-0`}>৳</span>
      <input
        type="number" min={0} value={value === 0 ? '' : value} placeholder="0"
        onChange={e => onChange(Math.max(0, Number(e.target.value) || 0))}
        className={`min-w-0 w-full text-right text-base font-black ${valueColor} bg-transparent border-none outline-none border-b border-dashed border-slate-200 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`}
      />
    </div>
  </div>
);
  ...
  return (
    <Card className="overflow-hidden border-2 border-border hover:border-primary/40 transition-all duration-300 bg-white dark:bg-card shadow-sm">
      <div className="p-4 flex flex-col gap-4">
        {/* Header */}
        <div className="flex items-center justify-between border-b pb-3 border-slate-100 dark:border-border">
          <div className="flex items-center gap-3">
            <div className="w-14 h-14 rounded-xl bg-slate-100 dark:bg-muted flex items-center justify-center border overflow-hidden shrink-0 p-1 shadow-sm">
              {brand.logo
                ? <img src={brand.logo} alt={brand.name} className="w-full h-full object-contain mix-blend-multiply dark:mix-blend-normal" />
                : <span className="text-lg font-black text-muted-foreground">{brand.name.substring(0, 2).toUpperCase()}</span>}
            </div>
            <div>
              <h3 className="font-extrabold text-xl leading-none text-slate-800 dark:text-foreground tracking-tight">{brand.name}</h3>
              <div className="flex items-center gap-2 mt-1.5">
                <select
                  className="text-xs font-black px-2 py-0.5 h-6 bg-slate-100 text-slate-800 border-slate-300 rounded outline-none ring-0 border"
                  value={localSize}
                ...
                <Badge className={`text-xs font-black border-0 px-2 py-0.5 h-6 ${regulatorType === '22mm' ? 'bg-orange-100 text-orange-600 hover:bg-orange-100' : 'bg-yellow-100 text-yellow-600 hover:bg-yellow-100'}`}>
                  {regulatorType}
                </Badge>
              </div>
            </div>
          ...
          <div className="flex gap-1">
            {(['22mm', '20mm'] as const).map(t => (
              <button key={t} onClick={() => onToggleRegulator(t)}
                className={`text-xs font-black px-3 py-1.5 rounded-lg border-2 transition-all ${regulatorType === t ? (t === '22mm' ? 'bg-orange-500 text-white border-orange-500' : 'bg-yellow-400 text-yellow-900 border-yellow-400') : 'bg-muted text-muted-foreground border-border hover:border-primary/40'}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="flex gap-3 items-stretch">
          <div className="w-20 shrink-0 flex items-center justify-center">
            <img src={cylinderImage} alt={`${brand.name} ${localSize}`} className="h-24 object-contain" onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
          </div>
        ...
        <div className="flex gap-3">
          <div className="flex-1 min-w-0 text-center px-1 py-1.5 bg-white dark:bg-card border border-green-100 rounded-lg shadow-sm flex flex-col justify-center overflow-hidden">
            ...
            <PriceRow label="BUY" value={activeRecord.prices.packaged.buying}    valueColor="text-rose-500"    onChange={v => upPrice('packaged', 'buying', v)} />
            <PriceRow label="RTL" value={activeRecord.prices.packaged.retail}    valueColor="text-emerald-600" onChange={v => upPrice('packaged', 'retail', v)} />
            <PriceRow label="WSL" value={activeRecord.prices.packaged.wholesale} valueColor="text-blue-600"    onChange={v => upPrice('packaged', 'wholesale', v)} />
          </div>
          <div className="flex-1 min-w-0 text-center px-1 py-1.5 bg-white dark:bg-card border border-cyan-100 rounded-lg shadow-sm flex flex-col justify-center overflow-hidden">
            ...
            <PriceRow label="BUY" value={activeRecord.prices.refill.buying}    valueColor="text-rose-500"    onChange={v => upPrice('refill', 'buying', v)} />
            <PriceRow label="RTL" value={activeRecord.prices.refill.retail}    valueColor="text-emerald-600" onChange={v => upPrice('refill', 'retail', v)} />
            <PriceRow label="WSL" value={activeRecord.prices.refill.wholesale} valueColor="text-blue-600"    onChange={v => upPrice('refill', 'wholesale', v)} />
          </div>
        </div>
      </div>
    </Card>
