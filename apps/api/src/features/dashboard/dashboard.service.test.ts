...
vi.mock('../inventory/inventory.model', () => ({
    StoreInventory: {
        find: vi.fn(),
    }
}));
...
describe('DashboardService', () => {
    ...
    describe('getStats', () => {
        it('should calculate stats correctly', async () => {
             ...
             expect(stats.totalDue).toBe(1000); // 1000
    ...
    describe('getInventorySummary', () => {
        it('should return low stock items', async () => {
            const mockCylinders = [
                { productId: { name: 'Brand A 12kg (20mm)' }, counts: { full: 2 } }
            ];
            const mockProducts = [
            ...
            (StoreInventory.find as any).mockReturnValue({
                limit: vi.fn().mockReturnValue({
                    populate: vi.fn().mockResolvedValue(mockCylinders)
                })
            });
