...
export class VehicleService {
  static async create(storeId: string, data: VehicleInput): Promise<IVehicle> {
    const normalizedModel = (data.vehicleModel || '').trim();

    const duplicate = await VehicleModel.findOne({
      storeId,
      licensePlate: { $regex: `^${data.licensePlate.trim()}$`, $options: 'i' },
      vehicleModel: normalizedModel,
    });
    if (duplicate) throw new Error(`A vehicle with plate "${data.licensePlate}"${normalizedModel ? ` and model "${normalizedModel}"` : ''} already exists`);

    const vehicle = await VehicleModel.create({ ...data, vehicleModel: normalizedModel, storeId });
    return vehicle;
  }
