...
describe('Transaction Routes', () => {
    ...
    it('should create a transaction with due amount and update wholesale Customer balance (B2B)', async () => {
        // 1. Create a Wholesale Customer
        const wholesaleCustomer = await CustomerModel.create({
            storeId: new mongoose.Types.ObjectId(storeId),
            type: 'wholesale',
            name: 'B2B Shop',
            phone: '01711111111',
        ...
        // Verify Wholesale Balance Update
        const updatedWholesaleCustomer = await CustomerModel.findById(wholesaleCustomer._id);
        expect(updatedWholesaleCustomer?.totalDue).toBe(5000);
    });
